...
exports.renderSolutions=async (req,res,next) => { // 특정 문제의 해설들을 가져오기
    try
    {
        ...
        const query='select nickname,count(user) as likes from (users inner join solutions on users.id=writer) left outer join like_table as l on solutions.id=l.solution where problem_id=? and solutions.deletedAt is null group by nickname order by likes DESC';
