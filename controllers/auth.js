...
exports.newUser=async (req, res, next) => {//새로운 회원 가입하기
  ...
  {//먼저 이메일부터 중복을 확인한다.
    ...
    if (exEmail) {//이미 존재하는 이메일일 경우
      res.status(404).send("E-Mail Already Exists.");
    }
    else
    {
      ...
      if(exNick)
      {//이미 존재하는 닉네임일 경우
        res.status(404).send("Nickname Already Exists.");
      }
      else
      {//모두 만족하면 데이터베이스에 등록한다.
        ...
        res.status(200).send("Successfully Created User Data.");
      }
    }
  } catch (error) {
    logger.error(error);
